...
module cutout_shape(pos = [0, 0, 0]) {
    ...
    // 首先减去一个带圆角的柱体
    translate([x, y, z])
        cuboid([button_hole_width, button_hole_length, baseHeight+separatorHeight*2], anchor=CENTER, rounding=2, except=[TOP,BOTTOM], $fn=16);
...
difference() {
    complete_holder();
    cutout_shape(button_hole_pos);
}

// 调试：半透明显示减去的形状以便定位问题
//for (i = [0:1]) %cutout_shape(button_hole_pos);
